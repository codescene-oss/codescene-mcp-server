...
fn install_claude_code_hooks(cfg: &HookConfig) -> Result<CallToolResult, ErrorData> {
    ...
    let msg = success_message(&InstallResult {
        ...
        hooks_description: "  - PreToolUse (Bash): Runs pre_commit_code_health_safeguard before git commits",
...
mod tests {
    ...
    async fn skips_duplicate_hooks() {
        ...
        .unwrap();
        let pre_arr = content["hooks"]["PreToolUse"].as_array().unwrap();
        assert_eq!(pre_arr.len(), 1, "should not duplicate hook groups");
    }
    ...
    async fn uses_custom_server_name() {
        ...
        .unwrap();
        let server_ref = content["hooks"]["PreToolUse"][0]["hooks"][0]["server"]
            .as_str()
            .unwrap();

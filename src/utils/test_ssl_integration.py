...
from .code_health_analysis import _is_cs_cli_command, cs_cli_path, run_local_tool
...
@unittest.skipUnless(
    is_cli_available(),
    "CS CLI not available - run TestSSLIntegrationWithDownload instead",
)
class TestSSLIntegration(unittest.TestCase):
    ...
    def test_cli_receives_ssl_args_in_command(self):
        ...
        try:
            ...
            with mock.patch("utils.code_health_analysis.subprocess.run", side_effect=mock_run), contextlib.suppress(Exception):

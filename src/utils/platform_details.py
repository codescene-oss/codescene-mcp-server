...
def get_ssl_cli_args() -> list[str]:
    ...
    ca_cert_path = os.getenv("REQUESTS_CA_BUNDLE") or os.getenv("SSL_CERT_FILE") or os.getenv("CURL_CA_BUNDLE")
...
def _create_truststore_from_pem(pem_path: str) -> str | None:
    ...
    try:
        ...
        truststore_path = os.path.join(tempfile.gettempdir(), f"cs-mcp-truststore-{cert_hash}.p12")

        # Only create if it doesn't exist
        if not os.path.exists(truststore_path):
            # Use serialize_java_truststore which is designed for CA-only truststores
            p12_data = pkcs12.serialize_java_truststore(certs=certs, encryption_algorithm=BestAvailableEncryption(b"changeit"))

...
def path_mismatch_error(file_path: Path, mount_path: Path) -> CodeSceneCliError:
    ...
    if idx is not None:
        ...
        suggestion = "file_path is not under CS_MOUNT_PATH. " "Check for typos or incorrect mount configuration."
    return CodeSceneCliError(f"file_path is not under CS_MOUNT_PATH: {str(file_path)!r}. {suggestion}")
...
def _relative_path_under_mount(file_path: Path, mount_path: Path) -> Path:
    ...
    try:
        ...
        raise path_mismatch_error(file_path, mount_path) from e
...
def get_relative_file_path_for_api(file_path: str) -> str:
    ...
    if os.getenv("CS_MOUNT_PATH"):
        ...
        return mounted_path.removeprefix("/mount/")

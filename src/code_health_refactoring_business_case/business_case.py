...
def _build_business_case_for(scenario, current_code_health):
    estimated_outcomes = metrics.collect(
        code_health_baseline=current_code_health,
        code_health_target=scenario["target_code_health"],
    )
    ...
    estimate = {
        ...
        "optimistic_outcome": {
            "defect_reduction": to_presentable(defect_optimistic),
            "development_speed_improvement": to_presentable(time_optimistic),
        },
        "pessimistic_outcome": {
            "defect_reduction": to_presentable(defect_pessimistic),
...
def _validate_code_health_score(name, value):
    ...
    if not _is_number(value):
        raise ValueError(f"{name} must be an int or float between 1.0 and 10.0 (got {type(value).__name__})")

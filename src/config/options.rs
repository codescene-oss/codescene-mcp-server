...
pub const OPTIONS: &[ConfigOption] = &[
    ...
    ConfigOption {
        ...
        description: "Optional CodeScene Cloud account/tenant ID for OAuth (positive integer). Pins multi-account users to a specific account. Use the switch_account tool to change accounts while signed in.",
        sensitive: false,
        hidden: false,
        api_only: true,
        aliases: &["account"],
    ...
    ConfigOption {
        ...
        env_var: "CS_OAUTH_ACCOUNT_ID",
        description: "Cloud account/tenant ID for the active OAuth session (managed automatically)",
        sensitive: false,
        hidden: true,
        api_only: false,
        aliases: &[],

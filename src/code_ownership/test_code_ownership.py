...
class TestCodeOwnership(unittest.TestCase):
    ...
    def test_code_ownership_none_found(self, mock_post):
        ...
        self.instance = CodeOwnership(FastMCP("Test"), {"query_api_list_fn": mocked_query_api_list})
    ...
    def test_code_ownership_some_found(self, mock_post):
        def mocked_query_api_list(*kwargs):
            return [
                {"owner": "some_owner", "path": "/some-path/some_file.tsx"},
                {"owner": "some_owner", "path": "/some-path/some_file2.tsx"},
                {"owner": "some_owner2", "path": "/some-path/some_file3.tsx"},
            ]
    ...
    @mock.patch.dict(
        os.environ,
        {"CS_MOUNT_PATH": "/some-path", "CS_ONPREM_URL": "https://onprem-codescene.io"},
    )
    @mock.patch("requests.post", side_effect=mocked_requests_post)
    def test_code_ownership_some_found_onprem(self, mock_post):
        def mocked_query_api_list(*kwargs):
            return [{"owner": "some_owner", "path": "/some-path/some_file.tsx"}]
    ...
    @mock.patch.dict(
        os.environ,
        {
            "CS_MOUNT_PATH": "/some-path",
            "CS_ONPREM_URL": "https://onprem-codescene.io/",
        },
    )
    @mock.patch("requests.post", side_effect=mocked_requests_post)
    def test_code_ownership_some_found_onprem_trailing_slash(self, mock_post):
        def mocked_query_api_list(*kwargs):
            return [{"owner": "some_owner", "path": "/some-path/some_file.tsx"}]
    ...
    def test_code_ownership_static_mode(self, mock_post, mock_get_path):
        ...
        def mocked_query_api_list(*kwargs):
            return [{"owner": "some_owner", "path": "src/some_file.tsx"}]

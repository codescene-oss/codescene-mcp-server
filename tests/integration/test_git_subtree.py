...
def run_subtree_tests_with_backend(backend: ServerBackend) -> int:
    ...
    with safe_temp_directory(prefix="cs_mcp_subtree_test_") as test_dir:
        ...
        results = [
            (
                "Code Health Score (subtree)",
                test_subtree_tool(ctx, subtree_prefix, "code_health_score", "utils.py"),
            ),
            (
                "Code Health Review (subtree)",
                test_subtree_tool(ctx, subtree_prefix, "code_health_review", "config.py"),
            ),
            (
                "Pre-commit Safeguard (subtree)",
                test_subtree_pre_commit(ctx, subtree_prefix),
            ),
            (
                "Absolute Paths (subtree)",
                test_subtree_absolute_paths(ctx, subtree_prefix),
            ),
...
def test_subtree_tool(ctx: ServerContext, subtree_path: str, tool: str, filename: str) -> bool:
    ...
    validator = (lambda r: _validate_score(r, "Code Health Score returned")) if is_score else (lambda r: _validate_review(r, "Review returned content"))

    return _run_mcp_test(ctx, ToolRequest(tool, test_file, validator), f"Subtree {tool} test")
...
def test_subtree_pre_commit(ctx: ServerContext, subtree_path: str) -> bool:
    ...
    try:
        ...
        response = client.call_tool(
            ...
            {"git_repository_path": str(ctx.repo_dir)},
            timeout=60,
        )
        result_text = extract_result_text(response)
...
def test_subtree_absolute_paths(ctx: ServerContext, subtree_path: str) -> bool:
    ...
    return _run_mcp_test(
        ...
        ToolRequest(
            "code_health_score",
            abs_path,
            lambda r: _validate_score(r, "Absolute path resolved"),
        ),
...
def test_main_repo_still_works(ctx: ServerContext) -> bool:
    ...
    return _run_mcp_test(
        ...
        ToolRequest(
            "code_health_score",
            test_file,
            lambda r: _validate_score(r, "Main repo file analysis works"),
        ),
        "Main repo file test",
    )

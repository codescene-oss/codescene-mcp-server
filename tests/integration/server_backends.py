...
- CargoBackend for running Cargo-built executables
...
class CargoBackend(ServerBackend):
    """Backend that uses a Cargo-built (Rust) executable."""
    ...
    def prepare(self) -> None:
        """Build the Rust executable with Cargo if not already provided."""
        if self.executable:
            print(f"\nUsing existing executable: {self.executable}")
            return

        print_header("Building Rust Executable with Cargo")
        ...
        binary_name = "cs-mcp.exe" if is_windows else "cs-mcp"
        ...
        # Copy to persistent location outside repo
        test_bin_dir = repo_root.parent / "cs_mcp_test_bin"
        test_bin_dir.mkdir(exist_ok=True)
        self.executable = test_bin_dir / binary_name
    ...
    def get_command(self, working_dir: Path) -> list[str]:
        """Return command to run the Cargo-built executable."""
...
class DockerBackend(ServerBackend):
    ...
    def prepare(self) -> None:
        """Build the Docker image with a test version tag.

        The Dockerfile accepts a VERSION build arg used as a git tag
        for version embedding at build time.
        """
...
class NpmBackend(ServerBackend):
    ...
    1. Builds the Cargo binary (or uses a pre-built one)
    ...
    def __init__(self, executable: Path | None = None):
        self._cargo_backend = CargoBackend(executable=executable)
    ...
    def prepare(self) -> None:
        ...
        # 1. Build the Cargo binary
        self._cargo_backend.prepare()
        binary_path = self._cargo_backend.executable
        assert binary_path is not None, "Cargo backend did not produce a binary"

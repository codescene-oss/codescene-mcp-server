...
def run_worktree_tests_with_backend(backend: ServerBackend) -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="cs_mcp_worktree_test_") as tmp:
        ...
        try:
            ...
            results = [
                ("Code Health Score", test_worktree_code_health_score_cmd(command, env, worktree_dir)),
                ("Code Health Review", test_worktree_code_health_review_cmd(command, env, worktree_dir)),
                ("Pre-commit Safeguard", test_worktree_pre_commit_cmd(command, env, worktree_dir)),
                ("Absolute Paths", test_worktree_relative_paths_cmd(command, env, worktree_dir)),
            ]
...
def test_worktree_code_health_score_cmd(command: list[str], env: dict, worktree_dir: Path) -> bool:
    """Test code_health_score in a git worktree (command version)."""
    print_header("Test: Code Health Score in Git Worktree")
    ...
    try:
        ...
        no_worktree_errors = all(err not in result_text.lower() for err in [
            "nonetype", "git_dir", "worktree error", "traceback"
        ])
        print_test("No worktree-related errors", no_worktree_errors)
...
def test_worktree_code_health_review_cmd(command: list[str], env: dict, worktree_dir: Path) -> bool:
    """Test code_health_review in a git worktree (command version)."""
    print_header("Test: Code Health Review in Git Worktree")
...
def test_worktree_pre_commit_cmd(command: list[str], env: dict, worktree_dir: Path) -> bool:
    """Test pre_commit_code_health_safeguard in a git worktree (command version)."""
    print_header("Test: Pre-commit Safeguard in Git Worktree")
...
def test_worktree_relative_paths_cmd(command: list[str], env: dict, worktree_dir: Path) -> bool:
    """Test absolute path resolution in git worktree (command version)."""

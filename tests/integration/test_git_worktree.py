...
def run_worktree_tests_with_backend(backend: ServerBackend) -> int:
    ...
    with safe_temp_directory(prefix="cs_mcp_worktree_test_") as test_dir:
        ...
        try:
            ...
            results = [
                ("Code Health Score", test_worktree_code_health_score(command, env, worktree_dir)),
                ("Code Health Review", test_worktree_code_health_review(command, env, worktree_dir)),
                ("Pre-commit Safeguard", test_worktree_pre_commit(command, env, worktree_dir)),
                ("Absolute Paths", test_worktree_absolute_paths(command, env, worktree_dir)),
            ]

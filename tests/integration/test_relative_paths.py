        ...
        if is_docker:
            ...
            results = [
                ...
                (
                    ...
                    test_absolute_path(command, env, repo_dir),
                ),
            ]
        else:
            ...
            results = [
                (
                    ...
                    test_relative_path_simple(command, env, repo_dir),
                ),
                (
                    "Relative Path (nested)",
                    test_relative_path_nested(command, env, repo_dir),
                ),
                (
                    "Relative Path with ./",
                    test_relative_path_dot_prefix(command, env, repo_dir),
                ),
                (
                    "Relative Path from subdir",
                    test_relative_path_from_subdir(command, env, repo_dir),
                ),
...
def test_relative_path_from_subdir(command: list[str], env: dict, repo_dir: Path) -> bool:
    ...
    ctx = ServerContext(command, env, repo_dir)
    config = PathTestConfig(
        "Relative Path from Subdirectory Context",
...
def test_mixed_slashes(command: list[str], env: dict, repo_dir: Path) -> bool:
    ...
    ctx = ServerContext(command, env, repo_dir)
    config = PathTestConfig(
        "Mixed Path Slashes (Windows Compatibility)",
